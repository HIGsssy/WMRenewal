...
impl ScreenManager {
    ...
    pub fn push(&mut self, mut screen: Box<dyn Screen>, state: &mut GameState) {
        self.widgets.clear();
        screen.init(&mut self.widgets, state);
        self.stack.push(screen);
    }

    pub fn pop(&mut self, state: &mut GameState) {
        self.stack.pop();
    ...
    pub fn pop_to(&mut self, id: ScreenId, state: &mut GameState) {
    ...
    pub fn process(&mut self, state: &mut GameState) -> ScreenAction {
        if let Some(screen) = self.stack.last_mut() {
            screen.process(&mut self.widgets, state)
        } else {
            ScreenAction::Quit
        }
    }

    pub fn on_event(&mut self, event: UiEvent, state: &mut GameState) -> ScreenAction {
        if let Some(screen) = self.stack.last_mut() {
            screen.on_event(event, &mut self.widgets, state)
        } else {
            ScreenAction::None
        }
    }

    pub fn handle_action(&mut self, action: ScreenAction, state: &mut GameState) {
        match action {
            ScreenAction::None => {}
            ScreenAction::Push(screen) => self.push(screen, state),
            ScreenAction::Pop => self.pop(state),
            ScreenAction::PopTo(id) => self.pop_to(id, state),
